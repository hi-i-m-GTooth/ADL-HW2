...
def parse_args():
    ...
    parser.add_argument(
        "--my_report",
        action="store_true",
        help="Record loss & em for each epoch.",
    )
...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        model.train()
        if args.with_tracking or args.my_report:
            total_loss = 0
        ...
        if args.checkpointing_steps == "epoch":
            ...
            if args.my_report:
                ...
                all_loss.append(total_loss.detach().float())
                all_em.append(eval_metric)
                logger.info((total_loss.detach().float(), eval_metric))
            accelerator.save_state(output_dir)
        ...
        if args.my_report:
            ...
            if accelerator.is_main_process:
                ...
                with open(os.path.join(args.output_dir, report), 'w') as f:
                    f.write(f"Loss: {all_loss}\n")
                    f.write(f"EM: {all_em}\n")
